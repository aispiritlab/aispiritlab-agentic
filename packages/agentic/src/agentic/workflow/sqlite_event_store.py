...
from agentic.workflow.messages import Message, normalize_recorded_message
        ...
        with _connect(self._path) as connection:
            ...
            connection.executemany(
                ...
                [
                    ...
                    for index, (message, payload) in enumerate(zip(recorded, serialized))
                ],
            )

            try:
                for projection in self._transactional_projections:
                    projection.project(connection, stream_name, recorded)
            except Exception:
                connection.rollback()
                raise
