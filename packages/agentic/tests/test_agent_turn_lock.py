...
class ConcurrencyProbeModel:
    ...
    def response(self, prompt: str, **_kwargs: object) -> ModelResponse:
        del prompt
        with self._lock:
            self.active += 1
...
class FakeProvider:
    ...
    @contextmanager
    def session(self, name: str = "model") -> Iterator[ConcurrencyProbeModel]:
        del name
        yield self._model


def _agent(model: ConcurrencyProbeModel, history: History | None = None) -> Agent:
    # FakeProvider is structurally what Agent uses (a `session()` context
    # manager), but it is not a ModelProvider subclass.
    return Agent(
        model_provider=cast("ModelProvider", FakeProvider(model)),
...
def test_distinct_agents_are_not_blocked_by_each_other() -> None:
    ...
    class RendezvousModel(ConcurrencyProbeModel):
        def response(self, prompt: str, **kwargs: object) -> ModelResponse:
            barrier.wait()
