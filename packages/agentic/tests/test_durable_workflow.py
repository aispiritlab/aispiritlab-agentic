...
from agentic.workflow.durable import DurableWorkflowExecutor
...
def test_duplicate_input_replays_same_outbox_without_repeating_decision() -> None:
    ...
    def decide(message: Event) -> tuple[Event, ...]:
        nonlocal calls
        calls += 1
    ...
    assert duplicate.outputs[0].metadata.message_id == first.outputs[0].metadata.message_id
...
class _ConflictOnceStore:
    ...
    def append_to_stream(self, stream_name, events, *, expected_version="NO_CONCURRENCY_CHECK"):
        if self.conflict:
            self.conflict = False
...
def test_occ_retry_reuses_decision_instead_of_repeating_external_call() -> None:
    ...
    executor = DurableWorkflowExecutor(store)  # type: ignore[arg-type]
    calls = 0
