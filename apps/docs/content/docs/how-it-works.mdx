...
publishes the next message back to Redis Streams. In distributed mode the same step is also
recorded in a durable per-turn workflow stream so a restarted service can replay stored outputs for
an already-processed input instead of running the handler twice.
...
6. Each distributed step is appended to a durable workflow stream keyed by turn ID.
