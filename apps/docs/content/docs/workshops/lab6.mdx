...
graph LR
    subgraph Docker Stack
        ...
        PL -.->|record SearchPlanned| ES[(Event Store)]
        PL -->|SearchRequested| RS
        RS -->|consume| SE[Search Service]
        SE -.->|record SearchResultsFetched| ES
...
- Before a service acks a consumed Redis record, it persists the input and outputs in a durable
  workflow stream. Redelivered inputs can then replay recorded outputs instead of re-executing the
  handler.
...
Each service records facts and sends commands separately. Facts stay in the workflow Event Store;
only commands with a target are published through Redis:

```python title="workshops/lab6/messages.py"
@dataclass(frozen=True, slots=True, kw_only=True)
class SearchPlanned(Event): ...       # fact, persisted, no target
class SearchRequested(UserCommand): ...  # intent, target="search"
class SearchResultsFetched(Event): ...   # fact, persisted, no target
class SummaryRequested(UserCommand): ... # intent, target="summary"
```
